...
def save_seperate_frame(frames, names, output_path):
    if names is None:
        names = [f"{i:06d}.png" for i in range(len(frames))]
    for i in range(len(names)):
        name = names[i]
        frame = frames[i]
        imageio.imwrite(os.path.join(output_path, name), frame)
...
@click.option("-dp", "--delta_path", type=click.Path(file_okay=True, dir_okay=False),
              required=True, help="path to the delta file")
...
def main(**kwargs) -> None:
    ...
    if config.camera_path == "thre360":
        ...
        _, poses, names = next(infinite_train_dl)
    ...
    delta = torch.load(config.delta_path)
    ...
    save_seperate_frame(animation_frames, names, output_path)
