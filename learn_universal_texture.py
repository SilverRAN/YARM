...
from thre3d_atom.modules.robust_trainer import train_multi_sh_vox_grid_vol_mod_with_posed_images_and_corruption
...
@click.option("-p", "--prompt", type=click.STRING, required=False, default="",
...
@click.option("--post_process_scc", type=click.BOOL, required=False, default=False,
              help="run post process strongly connected components",
               show_default=True)
@click.option("--proxy_model", type=click.STRING, required=False, default="resnet18",
                help="proxy model used for attention", show_default=True)
@click.option("--label", type=click.INT, required=False, default=404,
                help="label for the model", show_default=True)
@click.option("--delta_bound", type=click.FLOAT, required=False, default=5.0,
                help="delta bound for the model", show_default=True)
# fmt: on
# -------------------------------------------------------------------------------------
def main(**kwargs) -> None:
    ...
    for i in range(1, 10):
        if i == 1:
            ...
            pretrained_vol_mod, _ = create_volumetric_model_from_saved_model(
                model_path=Path(str(model_path).replace("01", "{:0>2d}".format(i))),
                thre3d_repr_creator=create_voxel_grid_from_saved_info_dict,
                device=device,
            )

        sds_vol_mod_list.append(pretrained_vol_mod)

    # train the model:
    train_multi_sh_vox_grid_vol_mod_with_posed_images_and_corruption(
        sds_vol_mod_list=sds_vol_mod_list,
