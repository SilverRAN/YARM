...
class PosedImagesDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        if self.directional:
            # return filename instead of index to facilitate saving the corresponding rendering result
            return image, unified_pose, direction, str(image_file_path).split('/')[-1]

        return image, unified_pose, str(image_file_path).split('/')[-1]
