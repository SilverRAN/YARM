...
from thre3d_atom.utils.constants import NUM_COLOUR_CHANNELS, INFINITY, NUM_ATTN_CHANNELS
...
def process_points_with_sh_voxel_grid(
    ...
    if parallel_points_chunk_size is None:
        interpolated_features = voxel_grid.forward(flat_sampled_points, delta)
    else:
...
def process_points_with_sh_voxel_grid_attn(
    sampled_points: SampledPointsOnRays,
    rays: Rays,
    voxel_grid: VoxelGrid,
    render_diffuse: bool = False,
    ...
    if render_diffuse:
        # if rendering the diffuse variant, then we only use the degree 0 features
        sh_coeffs = sh_coeffs.reshape(sh_coeffs.shape[0], NUM_ATTN_CHANNELS, -1)
        sh_coeffs = sh_coeffs[..., :1]
        sh_degree = 0
    else:
        # otherwise use all the features
