...
def defocus_blur(inp, disk_radius, alias_blur):
    ...
    X, Y = ch.meshgrid(mesh_range, mesh_range, indexing='ij')
...
def motion_blur(inp, k_radius, k_stdev, angle_offset=0):
    ...
    if ch.cuda.is_available():
        y_grid, x_grid = ch.meshgrid(ch.arange(h), ch.arange(w), indexing='ij')
        ...
        res = F.grid_sample(new_inp, grid, 'nearest', align_corners=True)
    else:
...
def zoom_blur(inp, max_zoom, zoom_step):
    ...
    for z in zooms:
        ...
        new_inp += zoomed[:,:,trim_top:trim_top + h, trim_top:trim_top + w]
